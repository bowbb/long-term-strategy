...
- 使用最新已完成交易日的信号，计算目标比例、目标金额和增加/减少金额；提交计算只读取最近一次刷新后保存的完整本地历史；
...
- 每个交易日判断一次。策略扫描完整的已完成历史：从低到高突破 `MA250 × 1.03` 时恢复 100% 基础仓位，从高到低突破 `MA250 × 0.97` 时降至 50% 基础仓位；在上下轨之间保持最近一次穿越后的 50% 或 100% 状态。
- 没有历史状态时，区间内按 100% 基础仓位初始化，低于下轨按 50% 基础仓位初始化；新增资金仍按当前目标比例投入，不会因滞回区间停留在现金。
- 使用最新已完成交易日的信号，在下一个交易日执行；有效数据下风险资产状态只有 50% 和 100% 两种。历史不足 250 个交易日时，该资产基础权重才转入境内长期国债。
...
每次官方刷新都会全量下载并整段替换对应本地历史，在 `data/refresh_log.json` 记录实际 provider、URL、检查时间、最新值、行数和序列 SHA-256；刷新失败时保留上一份通过质量检查的文件。手动计算不再访问网络，只读取最近一次成功或失败回退后保留的本地历史并扫描穿越。旧的比例校准逻辑已停用；兼容接口遇到明显尺度差异会拒绝写入，避免错误历史给后续数据定标。
